...
def animate(i):
	...
	graph_data = open('/home/ashfak/Desktop/DriversAssistanceSystem/deepgaze/FaceProject/test1.txt', 'r').read()
	...
	for line in lines:
		if len(line) > 1:
			...
			# rs.append(float(r))
			ps.append(float(p))
	# max_es = sum(es)/len(es)
	# print(max_es)
	# max_ms = sum(ms)/len(ms)
	# print(max_ms)
	# max_ps = sum(ps)/len(ps)
	# print(max_ps)
	#ax[0].clear()
	ax[0].plot(xs, aas, c='green')
	# plt.axhline(y=70, c="red")
	#ax[0].scatter(xs, aas, c='green', marker="o", s=20, picker=True)
	# fig.subplots_adjust(left=0.15)
	# fig.subplots_adjust(bottom=0.15)

	ax[0].set_ylabel('Attention')

	ax[1].plot(xs, es,  c='blue')
	# plt.axhline(y=70, c="red")
	#ax[1].scatter(xs, es, c='blue', marker="o", s=20, picker=True)
	# fig.subplots_adjust(left=0.15)
	# fig.subplots_adjust(bottom=0.15)

	ax[1].set_ylabel('EAR')

	ax[2].plot(xs, ms, c='yellow')
	# plt.axhline(y=70, c="red")
	#ax[2].scatter(xs, ms, c='yellow', marker="o", s=20, picker=True)
	# fig.subplots_adjust(left=0.15)
	# fig.subplots_adjust(bottom=0.15)
	ax[2].set_ylabel('Mouth')

	# ax[2].plot(xs, ys)
	# # plt.axhline(y=70, c="red")
	# ax[2].scatter(xs, ys, c=colors, marker="o", s=20, picker=True)
	# # fig.subplots_adjust(left=0.15)
	# # fig.subplots_adjust(bottom=0.15)
	# ax[2].set_ylabel('YAW')
	...
	# ax[3].plot(xs, rs)
	# # plt.axhline(y=70, c="red")
	# ax[3].scatter(xs, rs, c=colors, marker="o", s=20, picker=True)
	# # fig.subplots_adjust(left=0.15)
	# # fig.subplots_adjust(bottom=0.15)
	# ax[3].set_ylabel('Roll')

	ax[3].plot(xs, ps, c='red')
	# plt.axhline(y=70, c="red")
	#ax[3].scatter(xs, ps, c='red', marker="o", s=20, picker=True)
	# fig.subplots_adjust(left=0.15)
	# fig.subplots_adjust(bottom=0.15)
